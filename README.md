...
**Для развертывания на продакшн сервере** используйте скрипты из папки `deployment/`:

```bash
cd deployment
./setup_deploy.sh      # Настройка конфигурации
./deploy_all.sh        # Полное развертывание
```

Подробная документация: **[deployment/README.md](deployment/README.md)**
